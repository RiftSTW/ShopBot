...
module.exports = {
    async GenerateShop(data) {
        return new Promise(async(resolve, reject) => {
            ...
            let measure = ctx.measureText(date.toLocaleDateString('en-EN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })).width
            let left = (640 - (measure / 2))
            let stringDate = date.toLocaleDateString('en-EN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })
            ...
            while(e !== data[shopType].entries.length) {
                ...
                ctx.fillStyle = '#ffffff';
                if(data[shopType].entries[e].bundle !== null) finalName = "Pack of " + data[shopType].entries[e].items.length + " cosmetics"
